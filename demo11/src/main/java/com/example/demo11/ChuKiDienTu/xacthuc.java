...
public class xacthuc {
    ...
    static String dirname  = "F:\\\\" ;// "D:\\"
    static String fname  = "ts" ;
    ...
    public static  String name(String path) throws IOException {
//        path ="target\\demo11-1.0-SNAPSHOT\\file\\HoaDon24daky.pdf";
    ...
    public  String checkvalue(String path) throws IOException {
        KeyStore kall = PdfPKCS7.loadCacertsKeyStore();
        String s =null;
        PdfReader reader = new PdfReader(path);
        AcroFields af = reader.getAcroFields();
...
//                 + af.getTotalRevisions());
    ...
    public  String thoigian(String path) throws IOException {
        KeyStore kall = PdfPKCS7.loadCacertsKeyStore();
        String s =null;
        PdfReader reader = new PdfReader(path);
        AcroFields af = reader.getAcroFields();
        ...
        for (int k = 0; k < names.size(); ++k) {
            ...
            PdfPKCS7 pk = af.verifySignature(name);
            s= pk.getSigningCertificate().getNotBefore().toString() +" - " +pk.getSigningCertificate().getNotAfter().toString();
//		 System.out.println("Document revision: " + af.getRevision(name) + " of "
//                 + af.getTotalRevisions());
    ...
    public  String key(String path) throws IOException {
        KeyStore kall = PdfPKCS7.loadCacertsKeyStore();
        String s =null;
        PdfReader reader = new PdfReader(path);
        AcroFields af = reader.getAcroFields();
        ...
        for (int k = 0; k < names.size(); ++k) {
            ...
            PdfPKCS7 pk = af.verifySignature(name);
            s=  Base64.getEncoder().encodeToString(pk.getSigningCertificate().getPublicKey().getEncoded());
...
//                 + af.getTotalRevisions());
    ...
    public  String al(String path) throws IOException {
        KeyStore kall = PdfPKCS7.loadCacertsKeyStore();
        String s =null;
        PdfReader reader = new PdfReader(path);
        AcroFields af = reader.getAcroFields();
        ...
        for (int k = 0; k < names.size(); ++k) {
            ...
            PdfPKCS7 pk = af.verifySignature(name);
            s=pk.getSigningCertificate().getSigAlgName();
